...
export default {
      ...
      columns: [
        {
          ...
          width: 200,
          slots: { customRender: 'controller' }
        },
        {
          title: '接口列表',
          dataIndex: 'apis',
          width: 900,
          slots: { customRender: 'apis' }
        }
      ],
      // 菜单id
      menuId: null,
  ...
  methods: {
     ...
     */
    openWindow(title, businessType, businessId) {
      this.title = title;
      this.visible = true;
    ...
    async checkedGroup(event, record) {
      ...
      await MenuApi.addMenuResourceBind({ businessId: this.businessId, businessType: this.businessType, modularTotalResource, selectedResource });
