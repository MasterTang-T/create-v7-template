    ...
    getUserInfo() {
      if (setting.userUrl) {
        this.$http.get(setting.userUrl).then((res) => {
          ...
          // 注册消息通知的websocket
          try {
            let gunsWebsocket = new GunsWebsocket(setting.takeUser().wsUrl);
            gunsWebsocket.initWebSocket((result) => {
              if (setting.websocketMessageType.server.SYS_NOTICE_MSG_TYPE === result.serverMsgType) {
                this.$store.dispatch('notice/addNotice', result.data);
              }
            });
          } catch (e) {
            console.error(e);
